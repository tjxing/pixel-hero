...
type RegisterWriter = fn(&mut PPU, u8, &mut Rom) -> bool;
...
pub struct PPU {
    ...
    ppu_addr: PPUAddress,
    register_writers: [RegisterWriter; 8],
...
impl PPU {
    pub fn new(ctx: Option<CanvasRenderingContext2d>, rom: &Rom) -> PPU {
        PPU {
            ...
            register_writers: make_register_writers(),
    ...
    pub fn read_register(&mut self, addr: u16, rom: &Rom) -> u8 {
        ...
        if index == 2 {
            ...
            panic!("$200{} is not readable.", index);
        }
    }

    pub fn write_register(&mut self, addr: u16, v: u8, rom: &mut Rom) -> bool {
        let index = addr & 0x07;
        (self.register_writers[index as usize])(self, v, rom)
    }
...
fn make_register_writers() -> [RegisterWriter; 8] {
    [
        // PPU_CTRL
        |ppu, value, _| -> bool {
            if !ppu.wait_cpu {
